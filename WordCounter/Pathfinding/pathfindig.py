            ...
            if lookPixels(x,y,imgPix)!=OBST:
                ...
                if y-1 in range(img.size[0]):
                    if lookPixels(x,y-1,imgPix)!=OBST:
                        registerVertex(i,i-nW)
                # Looking SE,SW,NW,NE
                if x+1 in range(img.size[0]) and y+1 in range(img.size[0]):
                    if lookPixels(x+1,y+1,imgPix)!=OBST:
                        registerVertex(i,i+1)
                if x-1 in range(img.size[0]) and y+1 in range(img.size[0]):
                    if lookPixels(x-1,y+1,imgPix)!=OBST:
                       registerVertex(i,i+nW)
...
def getDist(n1,n2):
    d1=node2Pix(n1)
    d2=node2Pix(n2)
    return ((d2[0]-d1[0])**2+(d2[1]-d1[1])**2)//2
...
def a_star_search(graph, start):
    ...
    while not frontier.empty():
        ...
        for next in graph[current]:
            new_cost = cost_so_far[current] + getDist(current,next)
            if next not in cost_so_far or new_cost < cost_so_far[next]:
                cost_so_far[next] = new_cost
                priority = new_cost + getDist(target,next)#heuristic(target, next)
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            # A : fillgraph
            if event.key == pygame.K_a: img = Image.open(mapPath); imgPix=img.load(); screen.fill((255,255,255)); vertex=vertexInit(); fillGraph3()
            # S : clear path
            ...
            if event.key == pygame.K_t:
                ...
                path=a_star_search(vertex,source)
                printPath(path)
