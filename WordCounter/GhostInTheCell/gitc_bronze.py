...
def get_not_mine(fact):
	return [f for f in fact if f[1]==-1 or f[1]==0]
...
while True:
	...
	if len(availFactories)!=0:
		...
		# Upgrade
		if len(overPop)!=0:
		    overPop = [f for f in overPop if f[3]!=3 and f[2] >= 15]
		    if len(overPop)!=0:
		        orders.append('INC {}'.format(overPop[0][0]))
		...
		pts_win = False
		if get_build_power(enemyFactories) < get_build_power(myFactories):
			pts_win = True
			printf('No need to fight')
		if ready_to_attack(myFactories,myUnits) and len(enemyFactories)!=0 and not pts_win:
			# Attack
			printf('Ready to attack !')
			targetFactories = []
			for f in myFactories:
				targetFactories +=get_neighbour(f,factories)
			myUnitsAvail = 0
			for f in myFactories:
				myUnitsAvail += get_req_attack_power(f,units)
			...
			troopNumber = int(get_units_count(myFactories,myUnits)/(len(myFactories)+1)/len(myFactories))
			for f in myFactories:
				if f[0]!=targetFactories[0][0]:
					attack = get_req_attack_power(f,units)
					if get_req_attack_power(targetFactories[0],units,True)>0:
						orders.append('MOVE {} {} {}'.format(f[0],targetFactories[0][0],attack))
